  ...
  return (
    <div className="bg-teal-700 text-white min-h-screen flex items-center justify-center font-sans">
      <div className="container mx-auto px-4">
        <h1 className="text-5xl font-bold mb-12 text-center font-serif">
          Get In Touch
        </h1>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <ContactCard
            Icon={Phone}
            title="Phone"
            details={["Main: +8801708-562437"]}
          />
          <ContactCard
            Icon={Mail}
            title="Email"
            details={["freshfarm@gmail.com"]}
          />
        </div>
      </div>
    </div>
  ...
  return (
    <div className="bg-teal-600 p-8 rounded-lg shadow-lg text-center transition-all duration-300 hover:bg-teal-500 hover:shadow-xl">
      ...
      {details.map((detail, index) => (
        <p key={index} className="mb-2 text-teal-100">
          {detail}
        </p>
      ))}
    </div>
