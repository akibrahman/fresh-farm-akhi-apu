...
const PaymentSuccessful = () => {
  ...
  useEffect(() => {
    const verifyPayment = async () => {
      ...
      try {
        const response = await paymentSuccess({ orderID, sessionId }).unwrap();
        if (response.success) {
          await updatePaymentStatus({ orderID, status: "Success" }).unwrap();
          ...
          await updatePaymentStatus({ orderID, status: "Failed" }).unwrap();
